...
// Elements to be used
const iconEl = document.getElementsByClassName(".weather-icon");
const descEl = document.getElementsByClassName(".temperature-description");
  ...
  fetch(api)
    ...
    .then((data) => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const iconElement = document.getElementById("weather-icon-main-img");
        // Change image source
        iconElement.src = `/icons/${data["list"][0]["weather"][0]["icon"]}.png`;
        console.log(
          "Source = ",
          `/icons/${data["list"][0]["weather"][0]["icon"]}.png`
        );
        const temperatureDescriptionElement = document.getElementById(
          "temperature-description-main"
        );
        temperatureDescriptionElement.innerHTML = `<p>${data["list"][0]["weather"][0]["description"]}</p>`;

        //Today
        if (i == 0) {
          let day = document.getElementsByClassName("dayTempFirst");
      ...
      for (let i = 0; i < relevantImages.length; i++) {
        ...
        display.className = "relevantImage";
