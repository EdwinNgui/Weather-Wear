...
// function getEveryNth(data) {
//   const result = [];
//   let nth = 8;

//   for (let i = 0; i < data.length; i += nth) {
//     result.push(Number(data.list[i].main.temp - KELVIN).toFixed(0) + "°C");
//   }

//   //See all contents of data
...
//   let api = `api.openweathermap.org/data/2.5/forecast?lat=${latitude}&lon=${longitude}&exclude=minutely,hourly,alerts&appid=${key}`;
  let api = `http://api.openweathermap.org/data/2.5/forecast?lat=43.8335242&lon=-79.5585002&exclude=minutely,hourly,alerts&appid=997798eadb10e6a1dd80d06f23f607df`;
  fetch(api)
    .then((response) => response.json())
    .then((data) => {
      //Getting the temp for each day
      for (let i = 0; i < 5; i++) {
        const dayCount = 0; //Picks which day to assign to
        document.getElementById("dayTemp" + (dayCount + 1)).innerHTML =
          Number(data.list[i].main.temp - KELVIN).toFixed(0) + "°C";
        dayCount++;
        console.log("dayTemp" + (dayCount + 1));
        console.log(data);
        document.getElementById("testData").innerHTML = data.toString();
      }
    });
}
